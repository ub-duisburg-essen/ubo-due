...
>
  ...
  <xsl:template match="mods:mods" mode="dc">
    ...
    <xsl:call-template name="openaire_access_level" /> 
  </xsl:template>
  
  <xsl:template name="openaire_access_level">
    <dc:rights>
      <xsl:text>info:eu-repo/semantics/</xsl:text>
      <xsl:choose>
        <xsl:when test="mods:classification[contains(@valueURI,'/oa#')]">openAccess</xsl:when>
        <xsl:when test="mods:relatedItem[@type='host']/mods:classification[contains(@valueURI,'/oa#')]">openAccess</xsl:when>
        <xsl:otherwise>closedAccess</xsl:otherwise>
      </xsl:choose>
    </dc:rights>
  </xsl:template>
